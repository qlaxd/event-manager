...
const handleLogin = async () => {
  ...
  try {
    ...
    // Robust error handling for MFA (423) and other errors
    let status: number | undefined
    let detail: string | undefined
    if (error?.response) {
      status = error.response.status
      detail = error.response.data?.detail || error.response.data?.error_description
    } else if (error?.status) {
      status = error.status
      detail = error.data?.detail || error.data?.error_description
    }
